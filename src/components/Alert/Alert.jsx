...
const Alert = props => {
  ...
  return (
    <div
      className={`alert ${props.alertClass} alert-dismissible fade show`}
      role="alert"
    >
      <strong>
        {props.error ? "Holy guacamole an error occurred: " : "great: "}
      </strong>
      Check the information below below.
      <ul className="list-group mt-2">{msg}</ul>
      <button
        type="button"
        className="close"
  ...
  if (e.response) {
    ...
    serverError = (
      <div>
        {
          <li className="list-group-item">
            <strong>Error</strong>: Server Down!
          </li>
        }
      </div>
    );
